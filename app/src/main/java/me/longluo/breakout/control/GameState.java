...
import me.longluo.breakout.gl.GameSurfaceRenderer;
import me.longluo.breakout.model.Ball;
import me.longluo.breakout.model.BaseRect;
import me.longluo.breakout.model.BasicAlignedRect;
import me.longluo.breakout.model.Brick;
import me.longluo.breakout.model.OutlineAlignedRect;
import me.longluo.breakout.model.TexturedAlignedRect;
import me.longluo.breakout.util.SoundResources;
import me.longluo.breakout.util.TextResources;
...
public class GameState {
    ...
    public boolean restore() {
        synchronized (sSavedGame) {
            SavedGame save = sSavedGame;
            if (!save.mIsValid) {
                Timber.d("No valid saved game found");
                reset();
    ...
    public void allocBricks() {
        ...
        if (false) {
            ...
            Timber.d("max score on 'normal' is " + (max * 100));
        }
     ...
     */
    public void movePaddle(float arenaX) {
        /*
    ...
    public void calculateNextFrame() {
        ...
        if (curDeltaSec > MAX_FRAME_DELTA_SEC) {
            ...
            Timber.d("delta time was " + curDeltaSec + ", capping at " + MAX_FRAME_DELTA_SEC);
        ...
        switch (mGamePlayState) {
            ...
            default:
                Timber.e("GLITCH: bad state " + mGamePlayState);
                break;
        }
    ...
    private int moveBall(double deltaSec) {
        ...
        while (distance > 0.0f) {
                ...
                if (hit == null) {
                    ...
                    if (GameSurfaceRenderer.EXTRA_CHECK) {
                        if (mHitDistanceTraveled <= 0.0f) {
                            Timber.e("GLITCH: collision detection didn't move the ball");
                    ...
                    if (DEBUG_COLLISIONS) {
                        Timber.d("COL: intermediate cx=" + newPosX + " cy=" + newPosY);
                    }
                    ...
                    switch (mHitFace) {
                        ...
                        case HIT_FACE_NONE:
                        default:
                            Timber.e("GLITCH: unexpected hit face" + mHitFace);
                            break;
                    }
                    ...
                    if (DEBUG_COLLISIONS) {
                        Timber.d("COL: remaining dist=" + distance + " new dirX=" +
            ...
            if (hits == 0) {
                ...
                if (DEBUG_COLLISIONS) {
                    Timber.d("COL: none (dist was " + distance + ")");
                }
            ...
            for (int i = 0; i < numRects; i++) {
                ...
                if (circleX <= rectXScaleHalf) {
                    ...
                    if (dirXSign == cornerXSign && dirYSign == cornerYSign) {
                        ...
                        Timber.w("COL: impossible corner hit");
                        faceHit = HIT_FACE_SHARPCORNER;
                        msg = "???";
                    }

                    if (DEBUG_COLLISIONS) {
                        Timber.d("COL: " + msg + "-corner hit xd=" + xdist + " yd=" + ydist
                ...
                if (DEBUG_COLLISIONS) {
                    ...
                    Timber.d("COL: " + msg + " hit " + rect.getClass().getSimpleName() +
                ...
                if (faceToAdjust == HIT_FACE_HORIZONTAL) {
                    ...
                    Timber.w("GLITCH: unexpected faceToAdjust " + faceToAdjust);
                    hitXAdj = hitYAdj = 0.0f;
                }

                if (DEBUG_COLLISIONS) {
                    Timber.d("COL:  r=" + radius + " trav=" + traveled +
